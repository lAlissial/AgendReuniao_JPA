...
public class Fachada {
	...
	public static Reuniao criarReuniao (String datahora, String assunto, List<String> nomes) throws Exception{
		...
		if (nomes.size()<2) {
			...
			for(String nomezito: nomes) {
				try {
					...
					if (e.getMessage().contains("já está em outra reunião nesse horário")) {
						contador1++;
						seraqchegou3 = 1;
	...
	public static void apagarParticipante(String nome) throws Exception {
		...
		List<Reuniao> guardandoReunioeszitas = new ArrayList<Reuniao>();
		...
		if (guardandoReunioeszitas.size() > 0) {
